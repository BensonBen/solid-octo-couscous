...
export class AuthService {
	...
	public readonly createAccount = async (userRequest: Readonly<NewUserRequest>): Promise<string> => {
		...
		const user: Omit<User, 'password'> = _omit(await this.authDataProvider.createAccount(userRequest), [
			'password',
		] as Array<keyof User>);

		return Promise.resolve(
			jwtSign(user, process?.env?.AUTH_API_JWT_KEY, {
				algorithm: 'HS256',
	...
	public readonly login = async (loginRequest: Readonly<LoginUserRequest>): Promise<string> => {
		...
		const user: Omit<User, 'password'> = _omit(await this.authDataProvider.login(loginRequest), [
			'password',
		] as Array<keyof User>);
