...
export class ProductController {
    ...
    try {
      ...
      if (!_isEmpty(error)) {
        ...
        this.sqlLiteDataBase.database.serialize(() => {
          const query = this.sqlLiteDataBase.productFindOnPrimaryKey.replace(
            '?',
            params.productId
          );
  ...
  public readonly update = ({ body }: Request, response: Response) => {
    try {
      ...
      if (!_isEmpty(error)) {
        ...
        this.sqlLiteDataBase.database.serialize(() => {
          const query = this.sqlLiteDataBase.database.productFindOnPrimaryKey.replace(
            '?',
            body.id
          );
          ...
          this.sqlLiteDataBase.database.get(query, (error: Error | null, row) => {
            ...
            const stmt = this.sqlLiteDataBase.database.prepare(
              this.sqlLiteDataBase.productUpdateStatement
            );
  ...
  public readonly remove = async ({ params }: Request, response: Response) => {
    try {
      const { error } = await this.productIdValidator.validateAsync(
        params
      );
      ...
      if (!_isEmpty(error)) {
        ...
        this.sqlLiteDataBase.database.serialize(() => {
          ...
          const stmt = this.sqlLiteDataBase.database.prepare(
            this.sqlLiteDataBase.productDeleteOnPrimaryKey
          );

          console.log(
            green(
              `[Server] executing query: ${this.sqlLiteDataBase.productDeleteOnPrimaryKey}`
            )
          );
    ...
    try {
      ...
      if (!_isEmpty(error)) {
        ...
        this.sqlLiteDataBase.database.serialize(() => {
          ...
          console.log(
            green(
              `[Server] executing query: ${this.sqlLiteDataBase.productInsertStatement}`
            )
          );
          const stmt = this.sqlLiteDataBase.database.prepare(
            this.sqlLiteDataBase.productInsertStatement
          );
          ...
          console.log(
            green(
              `[Server] executing query: ${this.sqlLiteDataBase.productFindOnMaxPrimaryKey}`
            )
          );
